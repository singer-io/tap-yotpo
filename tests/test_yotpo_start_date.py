    ...
    def start_date_test(self, expected_strs, start_date=None, reduce_epoch=False):
        ...
        for stream in expected_streams:
            with self.subTest(stream=stream):
                ...
                primary_keys_list_1 = [message.get('data').get(expected_pk) for expected_pk in expected_primary_keys
                                       for message in synced_records_1.get(stream).get('messages')
                                       if message.get('action') == 'upsert']
                
                primary_keys_list_2 = [message.get('data').get(expected_pk) for expected_pk in expected_primary_keys
